...
class Round:
    ...
    def calculate_result(self) -> int:
        ...
        my_points = {
            me_playing_rock: 1,
            me_playing_paper: 2,
            me_playing_scissors: 3,
        }[self.my_choice]
        total_points_in_the_round += my_points
        if (self.my_choice, self.elf_choice) in [(me_playing_rock, elf_playing_scissors),
                                                 (me_playing_paper, elf_playing_rock),
                                                 (me_playing_scissors, elf_playing_paper)]:
            total_points_in_the_round += 6
            log.debug("I won")
        elif (self.my_choice, self.elf_choice) in [(me_playing_rock, elf_playing_rock),
                                                   (me_playing_paper, elf_playing_paper),
                                                   (me_playing_scissors, elf_playing_scissors)]:
