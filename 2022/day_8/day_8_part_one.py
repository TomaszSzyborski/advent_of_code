...
def process_raw_data_to_grid(forest: list[str]):
    return [[int(tree) for tree in line] for line in forest]
...
class Grid:
    ...
    def get_column_from_grid(self, column_number: int) -> list[int]:
        return [row[column_number] for row in self.data]

    def get_row_from_grid(self, row_number: int) -> list[int]:
        return self.data[row_number]


def is_tree_visible(tree_under_analysis: int, line_of_trees: list[int]) -> bool:
...
def count_visible_trees_in_the_forest(forest: Grid):
    visible_trees = 0
...
if __name__ == '__main__':
    ...
    visible_trees = count_visible_trees_in_the_forest(forest)
    print(visible_trees)
