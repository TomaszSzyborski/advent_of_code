...
"""
from day_8_part_one import read_data_file, process_raw_data_to_grid, Grid

def look_around(function):
    def wrapper(*args, **kwargs):
        print("looking started".center(80, "="))
        result = function(*args, **kwargs)
        print("looking finished".center(80, "="))
        return result
    return wrapper


@look_around
def number_of_trees_seen(tree_under_analysis: int, line_of_trees, **kwargs) -> int:
    print(f"{kwargs}")
    print(f"{line_of_trees=}")
    for count, tree in enumerate(line_of_trees, start=1):
        if tree >= tree_under_analysis:
            i_see_trees_of = count
            break
    else:
        i_see_trees_of = len(line_of_trees)
    print(f"{i_see_trees_of=}")
    return i_see_trees_of


def test_tree_with_brest_view():
    ...
    for row_index, tree_row in enumerate(forest.data):
        for tree_index, tree in enumerate(tree_row):
            print("INVESTIGATING TREE INITIATED".center(80,"#"))
            print(f"investigating {tree} at [{row_index}][{tree_index}]")
            row_of_trees = forest.get_row_from_grid(row_index)
            column_of_trees = forest.get_column_from_grid(tree_index)
            left = number_of_trees_seen(tree_under_analysis=tree,
                                        line_of_trees=row_of_trees[:tree_index][::-1],
                                        look="left")
            right = number_of_trees_seen(tree_under_analysis=tree,
                                         line_of_trees=row_of_trees[tree_index + 1:],
                                         look="right")
            up = number_of_trees_seen(tree_under_analysis=tree,
                                      line_of_trees=column_of_trees[:row_index][::-1],
                                      look="up")
            down = number_of_trees_seen(tree_under_analysis=tree,
                                        line_of_trees=column_of_trees[row_index + 1:],
                                        look="down")
            view = right * left * up * down
            print(f"{view=}")
            if view > total_tree_view:
                total_tree_view = view
            print("INVESTIGATING TREE COMPLETED".center(80,"#"))
    print()
    print(f"{total_tree_view=}")
    assert total_tree_view == 8
