...
The preparations are finally complete; you and the Elves
 leave camp on foot and begin to make your way toward the star fruit grove.
...
(your puzzle input); your subroutine needs to identify the first
position where the four most recently received characters were all different.
...
def test_finding_buffer_marker():
    window_size = 4
    ...
    assert next(buffer_starting_index_generator) + window_size == 7


test_data = [("mjqjpqmgbljsphdztnvjfqwrcgsmlb", 19),
...
@pytest.mark.parametrize("data_stream, expected_value", test_data)
def test_finding_message_markers(data_stream, expected_value):
    window_size = 14
    buffer_starting_index_generator = find_buffer_starting_marker(data_stream, window_size)
    assert next(buffer_starting_index_generator) + window_size == expected_value
