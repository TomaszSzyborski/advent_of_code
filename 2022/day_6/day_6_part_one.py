...
def find_buffer_starting_marker(data_stream: str, window_size: int):
    for index in range(0, len(data_stream)):
        chunk = data_stream[index:index + window_size]
        log.debug(set(chunk))
        log.debug(chunk)
...
if __name__ == '__main__':
    ...
    print(next(buffer_starting_index_generator) + window_size)
    # for index in buffer_starting_index_generator:
    #     log.info(index)
