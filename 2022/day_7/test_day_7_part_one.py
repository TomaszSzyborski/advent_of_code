...
save the resulting terminal output (your puzzle input). For example:
$ cd /
$ ls
dir a
14848514 b.txt
...
be to find directories that are good candidates for deletion.
...
from day_7_part_one import File
...
def test():
    puzzle_input_file_path = "test_input.txt"
    ...
    print(folder_weight)
    max_folder_weight = 100_000
    recovered_disk_space = 0
    for folder_to_delete, files_weight_list in folder_weight.items():
        total_folder_weight = sum(files_weight_list)
        print(f"{total_folder_weight=}")
        if total_folder_weight <= max_folder_weight:
            print(folder_to_delete)
            recovered_disk_space += total_folder_weight


    print(f"{recovered_disk_space=}")
    assert recovered_disk_space == 95437
