...
but they need your help finding the errors. Every item type is identified by
a single lowercase or uppercase letter (that is, a and A refer to different types of items).
...
for line in lines:
    rucksack = Rucksack(*split_in_half(line))
    log.debug(rucksack)
    rucksacks.append(rucksack)

priorities = {item: priority for priority, item
              in enumerate(string.ascii_letters, start=1)}
log.debug(priorities)

sum_of_priorities = 0
for rucksack in rucksacks:
    common = set(rucksack.first_compartment) & set(rucksack.second_compartment)
    log.debug(common)
    for element in common:
        priority = priorities[element]
        sum_of_priorities += priority
log.info(sum_of_priorities)
