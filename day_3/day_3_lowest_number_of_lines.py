...
def split_in_half(collection) -> tuple[str,str]:
    half = len(collection) // 2
    return collection[:half], collection[half:]


rucksacks: list[tuple[str, str]] = [split_in_half(line) for line in lines]
priorities = {item: priority for priority, item in enumerate(string.ascii_letters, start=1)}
sum_of_priorities = sum(priorities[(set(rucksack[0]) & set(rucksack[1])).pop()] for rucksack in rucksacks)
