...
log.debug(groups)
sets = [[set(item) for item in group] for group in groups]
log.debug(sets)
common_item_per_group = [set.intersection(*s) for s in sets]
log.debug(common_item_per_group)
priorities = {item: priority for priority, item in enumerate(string.ascii_letters, start=1)}
print(sum(priorities[item.pop()] for item in common_item_per_group))
