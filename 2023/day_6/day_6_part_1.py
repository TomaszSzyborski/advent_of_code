...
with open(file_name) as f:
    time, distance = [line.strip("Time:").strip("Distance:").strip().split() for line in f.read().split('\n')]
...
rides = [Ride(*r) for r in tuple(zip(time, distance))]
logging.info(f"{rides}")
wins = [r.calculate_times() for r in rides]
