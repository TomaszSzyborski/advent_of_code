...
def get_chunk_from_map(map: list[str], point: Point, offset_point: Point, max_size_of_data_chunk: int,
    ...
    if direction == "left":
        start = point.x - max_size_of_data_chunk + offset_point.x
        end = point.x + offset_point.x + 1
        selected_points = selected_line[start if start >= 0 else 0 : end if end<len(selected_line) else len(selected_line)]
    elif direction == "right":
        selected_points = selected_line[point.x + offset_point.x: point.x + max_size_of_data_chunk + offset_point.x]
    else:
