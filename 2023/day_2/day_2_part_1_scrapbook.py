...
def get_sets_from_game(raw_game_data: str, game_id: int):
    ...
    for game_set in game_sets:
        balls = [Ball(*ball_data.split(" ")) for ball_data in game_set.split(", ")]
...
if __name__ == '__main__':
    master_set = GameSet([Ball(12, Colour.RED), Ball(13, Colour.GREEN), Ball(14, Colour.BLUE)])
    games = strip_game_prefixes(data)
    valid_games = []
    for game in games:
        logging.debug(game)
        for game_set in game.game_sets:
            logging.debug(game_set)
            logging.debug(game_set > master_set)
