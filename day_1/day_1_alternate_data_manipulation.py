...
a minimum of fifty stars by December 25th.
...
with open("input.txt", 'r') as f:
    calories_data = f.read().splitlines()
    # equal to but neater than list(map(str.strip, f.readlines()))
log.debug(f"{calories_data=}")

total_calories_per_elf = [sum(map(int, chunk)) for chunk
                          in split_at(calories_data, lambda data: data == '')]
